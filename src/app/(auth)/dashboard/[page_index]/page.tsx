...
const PAGE_SIZE = 2; // 1ページに表示するアイテム数
...
const DashBoard: React.FC<DashBoardProps> = async ({ params }) => {
  ...
  return (
    <>
      <div className='w-full h-[80%] md:h-[85%] flex items-center justify-center '>
        <div className='size-full grid grid-cols-1 md:grid-cols-4 grid-rows-2 gap-y-10 gap-x-5 p-5'>
          {keeps &&
            keeps.map((keep) => (
      ...
      {keeps &&
        <Pagination length={keeps.length} currentPage={currentPage} pageSize={PAGE_SIZE} totalKeeps={totalKeeps} />
      }
      <RegisterButtonSp/>
    </>
  );
